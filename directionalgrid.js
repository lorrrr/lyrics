...
function showLyric(val) {
   textSize(8);
   var fonts = [ "Helvetica", "serif","monospace", "cursive"  ]

   //textFont(fonts[Math.floor(Math.random()*fonts.length)]);
  var lyric = val;
  var vol=amp.getLevel();
    spectrum = fft.analyze();
  var angle=radians(map(max(spectrum),130,270,0,180));
  var font=Math.floor(map(((min(spectrum)+max(spectrum))/2),80,150,0,4));
  textFont(fonts[font]);
  //var angle=radians(map(((min(spectrum)+max(spectrum))/2),70,130,0,360));
  fill(0);
  x=(counter%6)*100;
  y=floor(counter/6)*53+80;
