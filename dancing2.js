...
function preload() {
  font=loadFont("Minipax-SemiBold .otf");
...
//translate(0,height/2);
  ...
  rotate(radians(90));
  text("Excerpt from Tom's Diner by Suzanne Vega, 1982");
  pop();
...
function showLyric(val) {
  ...
  fill(0);
  noStroke();
  x=(counter%2)*(textWidth(prev)+unit)+10;

  oy=floor(counter/2)*58+70;

  var noi=int(map(vol,0,0.2,1,12));
 console.log(textWidth(lyric));
  for (var i=0;i<noi;i++){
    var nx=random(x-10,x+textWidth(lyric));
